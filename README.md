...
- **Live-capacity math** — combines `/packages/{id}` (per-VPS resource footprint) with `/compute/hypervisors/groups/{id}/resources` (live per-hypervisor free/allocated) to compute qty across every group the product can be placed in. Storage matching is by **type code** (`pool.storageType`) across both the default mountpoint (`localStorage`) and every additional pool (`otherStorage[]`), so a package qualifies on every hypervisor that exposes matching storage no matter which of the two it is served from — and picks the largest-fit pool when several share the same type. Group-level IPv4 pool accounted for without double-counting.
...
- **Storage type mismatch → 0 for that hypervisor.** If the package targets storage type code `4` (mountpoint) but the hypervisor exposes no pool of that type, that hypervisor contributes zero capacity — not a guess at "maybe placement will work out." This is a filter on `storageType`, not on `pool.id`, and it is applied to the default mountpoint as well as to additional pools; identical type codes across different hypervisors all qualify, which is what makes multi-hypervisor mountpoint/datastore placement work.
